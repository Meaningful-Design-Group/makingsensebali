...
[`enclosure/previous-iterations/node-v2/`](enclosure/previous-iterations/node-v2/) documents Fab Lab Bali's own second-generation node — the same XIAO + HM3301 + BME680 electronics, laid side by side on one floor inside a compact triangular shell. It was built, deployed, and co-located against an SCK, and it failed on two counts: the ESP32's radio heated the BME680 through the divider wall, and a bottom-mounted air intake lagged real PM peaks. Both failure modes are written up there, with what a V3 has to do about them. Read it before designing a shell of your own — it's the cheapest way to not repeat those two mistakes. Its "V2" counts node generations and has nothing to do with `enclosure/archive/v2-lantern/`.
