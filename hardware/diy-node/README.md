...
| — | 3D-printed enclosure (PETG, not PLA) | Fab Lab Bali | PLA softens in Bali rooftop temperatures. PETG holds up. Five snap-fit parts, STLs and print guide: [`enclosure/node-v3.2/`](enclosure/node-v3.2/). |
...
**Stage 2 — Perfboard, USB-powered, indoor.** Solder onto a 4×6 cm matrix board (the size the printed enclosure assumes). Use **female headers** for the XIAO and the BME680 — they're the parts most likely to die from a wiring mistake or surge, and you want to swap without desoldering. For the Plus build, solder four wires to the HM3301 carrier's test pads rather than using the Grove cable — see [`enclosure/node-v3.2/`](enclosure/node-v3.2/) for how the module seats. Run it for 48 hours indoors next to a known reference (a phone's air quality app pointed at a window will do for a sanity check). Confirm readings are stable, the device doesn't reset, and MQTT reconnects after WiFi drops.

**Stage 3 — Enclosure, deployed.** 3D-print the PETG enclosure from [`enclosure/node-v3.2/`](enclosure/node-v3.2/) — Fab Lab Bali's Node V3.2. Five printed parts, **no screws**: a detent snap-fit top cover, cantilever hooks holding every internal part, and wall screw holes plus cable-tie slots moulded straight into the back of the body, so there is no separate bracket to print. Two body variants, one antenna port or two, depending on whether you are running LoRa. Print the covers that match your boards — the BME680 cover differs between the Bosch breakout and the Seeed Grove module, and the HM3301 cover carries the mainboard, so its hook spacing differs between the Grove Shield and the DIY PCB. The BoM, wiring for all three mainboard options and the assembly sequence are in that folder. Earlier designs are in [`enclosure/previous-iterations/`](enclosure/previous-iterations/), kept for their failure notes, not for building. Mount it under shade, never in direct sun. Power via a weatherproof USB supply or a 5V solar panel + buck converter (the latter is a separate project; start with wall power).

> **Read the airflow caveat before you trust the numbers.** The V3 body fixes exhaust re-suction but keeps the underside intake and leaves the BME680 in the same sealed volume as the radio — the two things the Node V2 co-location actually faulted. Until a V3.2 has run a week beside a reference station, treat its temperature and humidity as unverified and expect PM peaks to read low. [`enclosure/README.md`](enclosure/README.md) has the full picture.
...
[`enclosure/node-v2/`](enclosure/node-v2/) documents Fab Lab Bali's own second-generation node — the same XIAO + HM3301 + BME680 electronics, laid side by side on one floor inside a compact triangular shell. It was built, deployed, and co-located against an SCK, and it failed on two counts: the ESP32's radio heated the BME680 through the divider wall, and a bottom-mounted air intake lagged real PM peaks. Both failure modes are written up there, with what a V3 has to do about them. Read it before designing a shell of your own — it's the cheapest way to not repeat those two mistakes. Its "V2" counts node generations and has nothing to do with `enclosure/previous-iterations/archive/v2-lantern/`.
