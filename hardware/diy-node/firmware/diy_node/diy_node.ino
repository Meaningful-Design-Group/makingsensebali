...
//  3. SC_DEVICE_TOKEN has MOVED to secrets.h (gitignored). It is a write
//     credential and no longer lives in a tracked file
...
//  5. Fresh-flash WiFi wipe: every time you compile + Upload this sketch
//     from the Arduino IDE, saved WiFi credentials are cleared automatically
//     — no need to double-reset by hand right after flashing a new node.
//     Once it's running, ordinary reboots and power cycles do NOT wipe
//     WiFi again; only the next Upload (or a manual double-reset) does.
//     See "fresh-flash detection" below.
//
...
//   - Preferences              built into the ESP32 Arduino core — no
//                              separate install; used only to remember which
//                              firmware build last ran (see fresh-flash note)
//
...
// Device token now lives in secrets.h (gitignored). Copy secrets.h.example to
// secrets.h and fill it in. It is a WRITE credential - see that file.
#include "secrets.h"
...
WiFiManager wm;
Preferences prefs;   // NVS-backed key/value store, used only to remember which
                     // firmware build last ran. Separate namespace from
                     // WiFiManager's own NVS-backed WiFi credential storage —
                     // resetSettings() below does not touch this.

// Changes every time this sketch is recompiled and uploaded from the Arduino
// IDE (the .ino is always retranspiled/recompiled on Upload, so __DATE__ and
// __TIME__ are fresh each time) but stays constant across resets and power
// cycles of the same running binary. That difference is what lets us tell
// "freshly flashed" apart from "just rebooted" — see fresh-flash detection
// in setup().
...
bool forceConfigPortal = false;   // true if a fresh flash OR a double-reset was detected this boot
...
void provisionWiFi() {
  ...
  if (forceConfigPortal) {
    // Reason (fresh flash vs. double-reset) was already logged in setup().
    Serial.println("[wifi] clearing saved network and opening config portal");
    wm.resetSettings();
  }
...
void setup() {
  ...
  // --- Fresh-flash detection: wipe saved WiFi once per new Upload from
  // Arduino, then keep it across every ordinary reboot/power-cycle of that
  // same binary until the next Upload. ---
  prefs.begin("msb", false);
  String lastBuild = prefs.getString("fw_build", "");
  bool freshFlash = (lastBuild != String(FW_BUILD_ID));
  if (freshFlash) {
    prefs.putString("fw_build", FW_BUILD_ID);
    Serial.printf("[wifi] new firmware build detected (%s) — will clear saved WiFi\n", FW_BUILD_ID);
  }
  prefs.end();

  // --- Double-reset check: if the RTC flag from a previous boot is still
  // armed, this boot happened within DRD_TIMEOUT_S of the last one — treat
  // it as a deliberate double-tap of RESET and force reprovisioning. ---
  bool doubleReset = drdFlagArmed;
  drdFlagArmed = true;   // armed until loop() disarms it after the window
  if (doubleReset) {
    Serial.println("[wifi] double-reset detected — will clear saved WiFi");
  }

  forceConfigPortal = freshFlash || doubleReset;
