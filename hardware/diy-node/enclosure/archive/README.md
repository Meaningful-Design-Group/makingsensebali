...
**Not part of this lineage:** [`../../node-v2/`](../../node-v2/) is Fab Lab Bali's separately-numbered "Node V2" — a compact triangular shell for the same electronics, documented with its own field evaluation. That number counts node generations, not the enclosure designs above, so `v2-lantern` and `node-v2` are unrelated designs that happen to share a digit. The review-before-printing disclaimer applies there too, for its own reasons.
