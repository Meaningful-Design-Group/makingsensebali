#!/usr/bin/env python3
"""Making Sense Bali DIY node — v11 ORGANIC SHELL + BASE BREATHING (Basic + Plus).
...
can_w, can_d, can_h = 38.0, 16.4, 40.0        #   board+can depth = 1.6+16.4 = 18 mm total
hm_hole_d = 3.2
...
# ---- v11 airflow fix: separate PM bay (baffle) + balanced high exhaust ----
BAFFLE_Y = 8.0            # wall splitting back climate column from front PM bay (Plus)
PM_EX_Z = 70.0           # PM exhaust height on the front wall, high on the 80mm HM3301 can (+foot_h applied)
...
VARIANTS = {
    "basic": dict(FRONT=24.0, FRONT_IN=21.5, body_top=84.0, hm=False, lean=9.0),
    "plus":  dict(FRONT=40.0, FRONT_IN=37.5, body_top=110.0, hm=True, lean=2.0),
}
...
def build(name, cfg):
    ...
    hm = cfg["hm"]
    lean = cfg["lean"]          # front-face lean-back over the body height (organic); small on Plus (tall HM)
    H = body_top
    ...
    outer = loft([
        ...
        sect(foot_h + 0.62 * H, FRONT - lean * 0.4, HW, yb_o, R_OUT),
        sect(foot_h + H,        FRONT - lean,      HW, yb_o, R_OUT),
    ])
    ...
    cavity = loft([
        ...
        sect(foot_h + 0.62 * H, FRONT - lean * 0.4 - wall,   HW_IN, yb_i, R_IN),
        sect(foot_h + H + 3,    FRONT - lean - wall,         HW_IN, yb_i, R_IN),
    ])
    ...
    # ---- v11 PM isolation (Plus): baffle + SEPARATE HM intake/exhaust ----
    pm_ex_area = 0.0
    if hm:
        # full-height baffle seals the front PM bay from the back climate column
        body = largest_solid(body + box_lh(-(HW_IN - 0.5), HW_IN - 0.5, BAFFLE_Y - wall / 2, BAFFLE_Y + wall / 2, foot_h - 1, body_top - 8 + foot_h))
        # PM exhaust on the front wall ABOVE the can — separate from the can-face intake,
        # so the HM3301 stops re-ingesting its own exhaust (the main airflow-review fix)
        for ez in (PM_EX_Z + foot_h, PM_EX_Z + 6 + foot_h):
            body = largest_solid(body - Pos(0, FRONT - 1.0, ez) * Box(20.0, wall + 6.0, 3.0, align=(Align.CENTER, Align.MAX, Align.CENTER)))
            pm_ex_area += 20.0 * 3.0

    # ---- climate chimney HIGH exhaust: enlarged, both sides, pushed to the very top ----
    # Area comparable to the base intake's OPEN area; sits behind the baffle on Plus.
    clim_ex_area = 0.0
    for sgn in (-1, 1):
        for i in range(3):
            lz = body_top - 6 - i * 6 + foot_h
            body = largest_solid(body - Pos(sgn * (HW - 1.0), -4.0, lz) * Rot(0, sgn * 40, 0) * Box(wall + 10.0, 20.0, 3.5, align=(Align.CENTER, Align.CENTER, Align.CENTER)))
            clim_ex_area += 20.0 * 3.5
    ...
    # ---- climate base intake + drop-in gyroid panel seat ----
    # Plus: climate opening = BACK zone only (behind the baffle); the FRONT zone gets a
    # SEPARATE PM intake grille. Basic: one climate opening (no PM bay).
    open_w = 2 * HW_IN - 8.0
    if hm:
        cl_y0, cl_y1 = -back_inner + 3.0, BAFFLE_Y - 2.0
    else:
        full_d = (back_inner + FRONT_IN) - 10.0
        cl_y0, cl_y1 = cap_cy - full_d / 2, cap_cy + full_d / 2
    open_d = cl_y1 - cl_y0
    open_cy = (cl_y0 + cl_y1) / 2
    base = largest_solid(base - box_lh(-open_w / 2, open_w / 2, cl_y0, cl_y1, base_bottom - 1, foot_h + 1))
    ...
    intake_open_area = open_w * open_d
    # PM fresh intake (Plus): separate down-facing grille in the FRONT base zone, under the HM
    if hm:
        for i in range(5):
            gx = -16 + i * 8
            base = largest_solid(base - box_lh(gx - 1.5, gx + 1.5, BAFFLE_Y + 2.0, FRONT - 4.0, base_bottom - 1, foot_h + 1))
    ...
    exports = [
        (f"v11_{name}_body", body, OUT, True, True),
        (f"v11_{name}_hood", hood, OUT, True, True),
        (f"v11_{name}_base", base, OUT, True, True),
        (f"v11_{name}_assembly", assembly, REVIEW, False, False),
        (f"v11_{name}_cutaway", cutaway, REVIEW, False, False),
    ]
    ...
    eff_intake = intake_open_area * 0.48   # gyroid panel ~48% open
    print(f"  {name} climate intake open {intake_open_area:.0f} mm2 (~{eff_intake:.0f} effective) "
          f"vs HIGH exhaust {clim_ex_area:.0f} mm2  -> {'BALANCED' if clim_ex_area >= eff_intake else 'intake-heavy'}")
    if hm:
        print(f"  {name} PM bay: isolated by baffle; separate front exhaust {pm_ex_area:.0f} mm2 + base PM intake grille")
...
if __name__ == "__main__":
    ...
    print(f"\n=== v11 STAGING build complete | interference {'OK' if allok else 'FAIL'} ===")
    print("v11 = airflow fix (separate PM bay + balanced high exhaust). NOT FOR PRINT — fill ICD 5.1;")
    print("confirm HM3301 envelope + can-port mapping by caliper; organic surfacing deferred to Rhino.")
